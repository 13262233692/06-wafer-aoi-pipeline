...
class TensorRTEngine:
    ...
    def infer_sync(
        ...
        Args:
            input_batch: Host-side input batch (N, C, H, W) float32
            d_input: Device pointer for input
            d_output: Device pointer for output
            h_output: Host-side output buffer (numpy array)
            stream: Optional CUDA stream to use

        Returns:
            Inference output as numpy array
        """
        if not self._loaded:
        ...
        self._cuda.memcpy_htod_async(d_input, input_batch, use_stream)
        ...
        if self._context is not None:
            if self._input_shapes and self._input_shapes[0][0] != batch_size:
                ...
                self._context.set_input_shape(name, (batch_size,) + self._input_shapes[0][1:])
            self._context.execute_async_v2(bindings=bindings, stream_handle=use_stream.handle)

        self._cuda.memcpy_dtoh_async(h_output, d_output, use_stream)
